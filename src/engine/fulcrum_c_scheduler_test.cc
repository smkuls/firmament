...
// FulcrumScheduler class unit tests.
...
// The fixture for testing class FulcrumScheduler.
class FulcrumSchedulerTest : public ::testing::Test {
 protected:
  // You can remove any or all of the following functions if its body
  // is empty.

  FulcrumSchedulerTest() :
  ...
  virtual void SetUp() {
    ...
    obj_store_->Flush();
    WallTime wall_time;
    sched_.reset(new FulcrumScheduler(job_map_, res_map_, &res_topo_,
  ...
  // FulcrumScheduler.
  scoped_ptr<FulcrumScheduler> sched_;
...
// tasks.
TEST_F(FulcrumSchedulerTest, LazyGraphReductionTest) {
...
TEST_F(FulcrumSchedulerTest, FindRunnableTasksForJob) {
  // Simple, plain, 1-task job (base case)
  JobDescriptor* test_job = new JobDescriptor;
  JobID_t job_id = GenerateJobID();
  test_job->set_uuid(to_string(job_id));
...
TEST_F(FulcrumSchedulerTest, ObjectIDToReferenceDescLookup) {
...
TEST_F(FulcrumSchedulerTest, ProducingTaskLookup) {
  JobID_t job_id = GenerateJobID();
  TaskDescriptor td;
  td.set_uid(1);
...
TEST_F(FulcrumSchedulerTest, FindRunnableTasksForComplexJob) {
...
// Find runnable tasks for a slightly more elaborate task graph.
TEST_F(FulcrumSchedulerTest, FindRunnableTasksForComplexJob2) {
  // Somewhat more complex job with 3 tasks.
  JobDescriptor* test_job = new JobDescriptor;
  JobID_t job_id = GenerateJobID();
  test_job->set_uuid(to_string(job_id));
